...
var css = {"PercentText_ReactGaugeUltimate":"_styles-module__PercentText_ReactGaugeUltimate__2ZqBe","Dark_ReactGaugeUltimate":"_styles-module__Dark_ReactGaugeUltimate__2IsyC","LabelText_ReactGaugeUltimate":"_styles-module__LabelText_ReactGaugeUltimate__RPY5U","Needle_ReactGaugeUltimate":"_styles-module__Needle_ReactGaugeUltimate__36LeG","Circle_ReactGaugeUltimate":"_styles-module__Circle_ReactGaugeUltimate__ITSBs"};
...
var useDeepCompareEffect = function useDeepCompareEffect(callback, dependencies) {
  React.useEffect(callback, useDeepCompareMemo(dependencies));
};

var startAngle = -Math.PI / 1.35;
var endAngle = Math.PI / 1.35;
var defaultStyle = {
  width: '100%'
};
...
var Gauge = function Gauge(props) {
  ...
  var height = React.useRef({});
  var doughnut = React.useRef({});
  var needle = React.useRef({});
  var outerRadius = React.useRef({});
  var margin = React.useRef({});
  var container = React.useRef({});
  var nbArcsToDisplay = React.useRef(0);
  var colorArray = React.useRef([]);
  var arcChart = React.useRef(d3.arc());
  var arcData = React.useRef([]);
  var pieChart = React.useRef(d3.pie());
  var prevProps = React.useRef(props);
  React.useEffect(function () {
    setArcData(props, nbArcsToDisplay, colorArray, arcData);
    if (props.id) {
      container.current = d3.select("#" + props.id);
      initChart();
    }
  }, []);
  ...
  }, [props.nrOfLevels, props.arcsLength, props.colors, props.percent]);
  var initChart = function initChart(update, resize, prevProps) {
    var _classNames;
    if (resize === void 0) {
      resize = false;
    }
    ...
    needle.current = g.current.append('g').attr('class', classNames(css.Needle_ReactGaugeUltimate, (_classNames = {}, _classNames[css.Dark_ReactGaugeUltimate] = darkMode, _classNames)));
...
Gauge.defaultProps = {
  ...
  arcPadding: 0.01,
  arcWidth: 0.23,
  colors: ['#4C586B', '#318C35', '#DD7128', '#952222'],
  hideText: false,
  animate: true,
  animDelay: 500,
...
var renderChart = function renderChart(resize, prevProps, width, margin, height, outerRadius, g, doughnut, arcChart, needle, pieChart, svg, props, container, arcData) {
  ...
  arcChart.current.outerRadius(outerRadius.current).innerRadius(outerRadius.current * (1 - props.arcWidth)).cornerRadius(function (radius) {
...
var getColors = function getColors(props, nbArcsToDisplay) {
  ...
  var colorScale = d3.scaleLinear().domain([1, nbArcsToDisplay.current]).range([colors[0], colors[colors.length - 1]]).interpolate(d3.interpolateHsl);
  var colorArray = [];
...
var drawNeedle = function drawNeedle(resize, prevProps, props, width, needle, container, outerRadius, g) {
  ...
  var percent = props.percent,
    ...
    darkMode = props.darkMode;
  var needleRadius = 15 * (width.current / 450),
  ...
  needle.current.append('path').attr('d', pathStr).attr('class', 'needle');
  needle.current.append('circle').attr('cx', centerPoint[0]).attr('cy', centerPoint[1]).attr('r', needleRadius).attr('class', classNames(css.Circle_ReactGaugeUltimate, (_classNames2 = {}, _classNames2[css.Dark_ReactGaugeUltimate] = darkMode, _classNames2)));
  if (!hideText) {
    addText(percent, props, outerRadius, width, g);
  }
  if (!resize && animate) {
    needle.current.transition().delay(props.animDelay).ease(d3.easeElastic).duration(props.animateDuration).tween('progress', function () {
      var currentPercent = d3.interpolateNumber(prevPercent, percent);
...
var calculateRotation = function calculateRotation(percent, outerRadius, width) {
  var needleLength = outerRadius.current * 0.55,
    ...
    centerPoint = [0, -needleRadius / 2],
    topPoint = [centerPoint[0] - needleLength * Math.cos(theta) * 1.25, centerPoint[1] - needleLength * Math.sin(theta) * 1.25],
    leftPoint = [centerPoint[0] - needleRadius * Math.cos(theta - Math.PI / 1.35), centerPoint[1] - needleRadius * Math.sin(theta - Math.PI / 1.35)],
    rightPoint = [centerPoint[0] - needleRadius * Math.cos(theta + Math.PI / 1.35), centerPoint[1] - needleRadius * Math.sin(theta + Math.PI / 1.35)];
  var pathStr = "M " + leftPoint[0] + " " + leftPoint[1] + " L " + topPoint[0] + " " + topPoint[1] + " L " + rightPoint[0] + " " + rightPoint[1];
  return pathStr;
};
var scalePercent = function scalePercent(prevMin, prevMax, newMin, newMax) {
  var offset = newMin - prevMin,
    scale = (newMax - newMin) / (prevMax - prevMin);
  return function (percent) {
    return offset + scale * percent;
  };
};
var percentToRad = function percentToRad(percent) {
  return scalePercent(0, 1, -0.25, 1.25)(percent) * Math.PI;
};
var addText = function addText(percentage, props, outerRadius, width, g) {
  ...
  var formatTextValue = props.formatTextValue,
    ...
    darkMode = props.darkMode;
  var textPadding = width.current / 1.8;
  var text = formatTextValue ? formatTextValue(floatingNumber(percentage)) : floatingNumber(percentage) + '%';
  g.current.append('g').attr('class', 'text-group').attr('transform', "translate(" + outerRadius.current + ", " + (outerRadius.current / 2 + textPadding) + ")").append('text').text(text).style('font-size', function () {
    return fontSize ? fontSize : width.current / 7 / (text.length > 10 ? text.length / 10 : 1) + "px";
  }).attr('class', classNames(css.PercentText_ReactGaugeUltimate, (_classNames3 = {}, _classNames3[css.Dark_ReactGaugeUltimate] = darkMode, _classNames3)));
  g.current.append('g').attr('class', 'text-group').attr('transform', "translate(" + outerRadius.current + ", " + (outerRadius.current / 2 + (textPadding - textPadding / 3)) + ")").append('text').text(props.label).style('font-size', function () {
    return fontSize ? fontSize / 2 : width.current / 10 / (text.length > 10 ? text.length / 10 : 1) / 2 + "px";
  }).attr('class', classNames(css.LabelText_ReactGaugeUltimate, (_classNames4 = {}, _classNames4[css.Dark_ReactGaugeUltimate] = darkMode, _classNames4)));
};
var floatingNumber = function floatingNumber(value, maxDigits) {
  if (maxDigits === void 0) {
    maxDigits = 2;
  }
...
var updateDimensions = function updateDimensions(props, container, margin, width, height) {
  ...
  if (id) {
    var _document, _divDimensions, _divDimensions2;
    divDimensions = (_document = document) === null || _document === void 0 ? void 0 : _document.getElementById(id);
    divWidth = ((_divDimensions = divDimensions) === null || _divDimensions === void 0 ? void 0 : _divDimensions.clientWidth) || divWidth;
    divHeight = ((_divDimensions2 = divDimensions) === null || _divDimensions2 === void 0 ? void 0 : _divDimensions2.clientHeight) || divHeight;
  }
