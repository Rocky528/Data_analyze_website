...
import { arc, pie, select, scaleLinear, interpolateHsl, easeElastic, interpolateNumber } from 'd3';
...
const renderChart = (resize, prevProps, width, margin, height, outerRadius, g, doughnut, arcChart, needle, pieChart, svg, props, container, arcData) => {
  ...
  arcChart.current.outerRadius(outerRadius.current).innerRadius(outerRadius.current * (1 - props.arcWidth)).cornerRadius(radius => {
...
const drawNeedle = (resize, prevProps, props, width, needle, container, outerRadius, g) => {
  ...
  needle.current.append('circle').attr('cx', centerPoint[0]).attr('cy', centerPoint[1]).attr('r', needleRadius).attr('class', classNames(css.Circle_ReactGaugeUltimate, {
    [css.Dark_ReactGaugeUltimate]: darkMode
  }));
  if (!hideText) {
...
const scalePercent = (prevMin, prevMax, newMin, newMax) => {
...
const addText = (percentage, props, outerRadius, width, g) => {
  ...
  g.current.append('g').attr('class', 'text-group').attr('transform', `translate(${outerRadius.current}, ${outerRadius.current / 2 + textPadding})`).append('text').text(text).style('font-size', () => fontSize ? fontSize : `${width.current / 7 / (text.length > 10 ? text.length / 10 : 1)}px`).attr('class', classNames(css.PercentText_ReactGaugeUltimate, {
    [css.Dark_ReactGaugeUltimate]: darkMode
  }));
  g.current.append('g').attr('class', 'text-group').attr('transform', `translate(${outerRadius.current}, ${outerRadius.current / 2 + (textPadding - textPadding / 3)})`).append('text').text(props.label).style('font-size', () => fontSize ? fontSize / 2 : `${width.current / 10 / (text.length > 10 ? text.length / 10 : 1) / 2}px`).attr('class', classNames(css.LabelText_ReactGaugeUltimate, {
    [css.Dark_ReactGaugeUltimate]: darkMode
  }));
};
